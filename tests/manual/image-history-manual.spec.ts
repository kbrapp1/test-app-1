...
test.describe('Manual - Image History Panel', () => {
  ...
  test('History panel loading, scrolling, and caching behavior', async ({ page }) => {
    // 1. Pause for 5 seconds to allow for pre-loading, per user request.
    console.log(`[${new Date().toISOString()}] --- Starting 5-second pause... ---`);
    await page.waitForTimeout(5000);
    console.log(`[${new Date().toISOString()}] --- Pause finished. Opening history panel... ---`);
