...
vi.mock('@/lib/config/ttsProviderConfig', () => ({
...
const mockPredictionItem = (id: string, createdAt: string, outputUrl?: string | null, provider = 'replicate'): TtsPredictionRow => ({
  id,
  createdAt,
  inputText: `Input for ${id}`,
  voiceId: 'test-voice',
  status: 'succeeded',
  outputUrl: outputUrl === undefined ? `http://example.com/${id}.mp3` : outputUrl,
  ...
  replicatePredictionId: `replicate-${id}`,
  sourceAssetId: `source-${id}`,
});

// Consolidated mock items for all tests
const mockItems: TtsPredictionRow[] = Array.from({ length: 15 }, (_, i) => 
  mockPredictionItem(`id${i + 1}`, new Date(Date.now() - (i + 1) * 100000).toISOString())
);
...
describe('TtsHistoryPanel Item State Management', () => {
  ...
  it('does not call markTtsUrlProblematic if item is already problematic', async () => {
    const alreadyProblematicItem = { ...mockItems[0], is_output_url_problematic: true, output_url_last_error: 'Initial error' };
  ...
  it('does not update item locally if markTtsUrlProblematic action fails', async () => {
    const itemToMark = { ...mockItems[0], is_output_url_problematic: false }; // Ensure it's not already problematic
