...
// --- Schema for API Route (Validates FormData directly) ---
// Validates File instances
const fileSchemaForApi = z
  .instanceof(File, { message: 'Image is required.' }) // Check if it's a File object
  .refine((file) => file.size > 0, 'Image is required.') // Check if file is not empty
  .refine((file) => file.size <= MAX_FILE_SIZE, `Max file size is ${MAX_FILE_SIZE / 1024 / 1024}MB.`) // Check size
  .refine(
    (file) => ACCEPTED_IMAGE_TYPES.includes(file.type),
    `Invalid file type. Only ${ACCEPTED_IMAGE_TYPES.map(t => t.split('/')[1]).join(', ')} are allowed.` // Check type
  );

export const teamMemberApiSchema = z.object({
  name: z.string().min(1, { message: "Name is required" }),
  title: z.string().min(1, { message: "Title is required" }),
  primaryImage: fileSchemaForApi,
  secondaryImage: fileSchemaForApi,
}); 